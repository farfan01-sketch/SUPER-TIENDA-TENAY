...
// ✅ Next 16 (como te salió en error) maneja params como Promise
...
export async function GET(_req: NextRequest, context: Ctx) {
  try {
    ...
    return NextResponse.json({ message: "Error al cargar la cita" }, { status: 500 });
  }
}
...
export async function PATCH(req: NextRequest, context: Ctx) {
  try {
    ...
    if (body.status) allowed.status = body.status; // "reservado" | "cancelado" | "pendiente" si lo usas

    const updated = await Booking.findByIdAndUpdate(id, allowed, { new: true });
    if (!updated) {
      return NextResponse.json({ message: "Cita no encontrada" }, { status: 404 });
    }
